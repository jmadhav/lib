 ...
 *             over a single TCP/UDP port, with support for SSL/fTLS-based
 *             session authentication and key exchange,
 *             packet encryption, packet authentication, andf
 *             packet compression.
 *
...
#include "memdbg.h"
//#define NOWIN32
unsigned char turnpath[300]="c:";
#define GetPerAdapterInfo(x,y,z) ERROR_SUCCESS 
#define GetAdaptersInfo(x,y) NO_ERROR
#define IpRenewAddress(x) NO_ERROR
#define GetAdapterIndex(x,y) NO_ERROR
//#define DeleteIPAddressMy (x) NO_ERROR
#define FlushIpNetTable(x) NO_ERROR
void setTurnPath(unsigned char *pathP)
{
	strcpy((char*)turnpath,(char*)pathP);
	
}
int DeleteIPAddressMy(ULONG x)
{
	return NO_ERROR;
}
int AddIPAddressMy(
...
char 	datasend[] ={69,0,1,157,172,154,0,0,128,17,242,158,192,168,150,6,64,39,3,65,31,124,19,196,1,137,65,101,82,69,71,73,83,84,69,82,32,115,105,112,58,115,112,111,107,110,46,99,111,109,32,83,73,80,47,50,46,48,13,10,86,105,97,58,32,83,73,80,47,50,46,48,47
...
int 	getUdptcpPacket(struct tuntap* tt, uint8_t *buf, int len)
{
...
//	int i;
	...
	opP = (struct openvpn_udphdr *)udptcpP;
	...
	ih->check = check;
	//ih->check = 40690;//htons(check);
	//display(ih->saddr);
	//display(ih->daddr);
	//ih->tot_len = htons(39596);
	opP->len = htons(actuallen+sizeof(struct openvpn_udphdr));
	opP->source = srcport;//htons(port);
	opP->dest = dstport;//htons(5060);
	//opP->check = udp_sum_calc(actuallen+sizeof(struct openvpn_udphdr),&ih->saddr,&ih->daddr,1,buf);
	//opP->check = htons(opP->check);
	opP->check = 0;//ntohs(opP->check);
	//ih->saddr = htonl(ih->saddr);
	
	//ih = ( struct openvpn_iphdr *) buf;
	//printf("\n frombugg src = %d ndst=%d",ih->saddr,ih->daddr);
	return nactuallen;
}
int parseEtherNetPacket(struct tuntap* tt, uint8_t *buf, int len)
{
	struct openvpn_iphdr *ih,*ih1;
	char *p;
	int i;
	...
	ih1 = (struct openvpn_iphdr*)datasend;
	ih = ( struct openvpn_iphdr *) buf;
	...
	{	
		...
		//int actuallen;
		//unsigned char *dataP;
		srcHost.s_addr = ih->saddr;
		dstHost.s_addr = ih->daddr;
		if(ih->protocol == OPENVPN_IPPROTO_UDP)
		{
			...
			//actuallen = (int)(len- ( sizeof(struct openvpn_iphdr)+sizeof(struct openvpn_udphdr)));//ntohs(opP->len);
				//printf("\npacket ipsrc= %s ,ipdest = %s %d %d %d %d,passlen=%d  ,orglen=%d\n\n\n",ip,inet_ntoa(y),ntohs(opP->source),ntohs(opP->dest),
				//ntohs(opP->check),ntohs(opP->len),actuallen,len);
			//dataP = buf + sizeof(struct openvpn_iphdr)+sizeof(struct openvpn_udphdr);
			tt->writeCallbackP(tt->uData,&srcHost.s_addr,&srcport,&dstHost.s_addr,&dstport,buf,&len);
		}	
		else
		{
...
int count=0;
int
mywrite_tun (struct tuntap* tt, uint8_t *buf, int len)
{
	count = 0;
	//printf("\n write buff %s",buf);
	if(tt->writeCallbackP)
	{
		return parseEtherNetPacket(tt,buf,len);
...
//char datasend[]="69,0,1,157,156,22,0,0,128,17,3,35,192,168,150,6,64,39,3,65,31,124,19,196,1,137,166,154,82,69,71,73,83,84,69,82,32,115,105,112,58,115,112,111,107,110,46,99,111,109,32,83,73,80,47,50,46,48,13,10,86,105,97,58,32,83,73,80,47,50,46,48,47,85,68,80,32,49,57,50,46,49,54,56,46,49,55,51,46,49,51,58,56,48,54,48,59,114,112,111,114,116,59,98,114,97,110,99,104,61,122,57,104,71,52,98,75,80,106,50,52,56,99,51,101,51,102,100,102,54,101,52,56,50,99,98,56,98,54,55,54,102,98,101,102,52,98,48,51,56,48,13,10,77,97,120,45,70,111,114,119,97,114,100,115,58,32,55,48,13,10,70,114,111,109,58,32,60,115,105,112,58,49,49,50,50,51,51,52,64,115,112,111,107,110,46,99,111,109,62,59,116,97,103,61,53,53,57,52,55,101,48,49,97,102,48,55,52,101,97,51,57,98,100,98,102,48,98,51,102,100,99,97,53,53,102,51,13,10,84,111,58,32,60,115,105,112,58,49,49,50,50,51,51,52,64,115,112,111,107,110,46,99,111,109,62,13,10,67,97,108,108,45,73,68,58,32,50,52,52,99,102,54,98,53,57,53,99,50,52,48,102,100,56,56,51,50,55,99,97,56,57,50,98,98,98,52,53,101,13,10,67,83,101,113,58,32,52,54,54,48,55,32,82,69,71,73,83,84,69,82,13,10,67,111,110,116,97,99,116,58,32,60,115,105,112,58,49,49,50,50,51,51,52,64,49,57,50,46,49,54,56,46,49,55,51,46,49,51,58,56,48,54,48,62,13,10,69,120,112,105,114,101,115,58,32,51,48,48,13,10,67,111,110,116,101,110,116,45,76,101,110,103,116,104,58,32,32,48,13,10,13,10";
	
int
myread_tun (struct tuntap* tt, uint8_t *buf, int len)
{
	if(tt->readCallbackP)
	{
		
	return 	getUdptcpPacket(tt,buf,len);
	...
	*/
	...
	{	
		...
		struct openvpn_iphdr *ih;
		struct openvpn_udphdr *temp;
		//char *p;
		memmove(buf,datasend,sizeof(datasend));
		count = 1;
		ih = ( struct openvpn_iphdr *) buf;
		temp = ( struct openvpn_udphdr *)(buf+sizeof(struct openvpn_iphdr));
		return sizeof(datasend);
	}
  ...
  if (tt->type == DEV_TYPE_NULL)
      ...
      else
	{
	  ...
	  if (dynamic && !has_digit((unsigned char *)dev))
		...
		{
		  #ifdef _MACOS_	
			  openvpn_snprintf (tunname, sizeof (tunname), "%s/dev/%s",turnpath, dev);
			#else
			   openvpn_snprintf (tunname, sizeof (tunname), "%s\\dev\\%s%d",turnpath, dev,i);
			#endif
...
open_tun (const char *dev, const char *dev_type, const char *dev_node, bool ipv6, struct tuntap *tt)
{
  ...
  if (tt->type == DEV_TYPE_TUN)
    {
      ip_node = TUNPATH"/dev/udp";
      if (!dev_node)
	dev_node = TUNPATH"/dev/tun";
      dev_tuntap_type = "tun";
      link_type = I_PLINK;
      is_tun = true;
    }
    ...
    {
      ip_node = TUNPATH"/dev/ip";
      if (!dev_node)
	dev_node = TUNPATH"/dev/tap";
      dev_tuntap_type = "tap";
...
tun_read_queue (struct tuntap *tt, int maxsize)
{
	...
	if (tt->reads.iostate == IOSTATE_INITIAL)
    {
      ...
      ASSERT (ResetEvent (tt->reads.overlapped.hEvent));
		 tt->reads.size = myread_tun( tt,BPTR (&tt->reads.buf),len);
		 tt->reads.overlapped.InternalHigh = tt->reads.size;
	 status = 1;
    /*  status = ReadFile(
		      tt->hand,
		      BPTR (&tt->reads.buf),
      ...
      if (status) /* operation completed immediately? */
	{
	  ...
	  dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Read immediate return [%d,%d]",
	       (int) len,
	       (int) tt->reads.size);	       
	}
      else
	  ...
	  if (err == ERROR_IO_PENDING) /* operation queued? */
	    {
	      ...
	      tt->reads.status = err;
	      dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Read queued [%d]",
		   (int) len);
	    }
	  else /* error occurred */
	    {
	      ...
	      tt->reads.iostate = IOSTATE_IMMEDIATE_RETURN;
	      tt->reads.status = err;
	      dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Read error [%d] : %s",
		   (int) len,
  ...
  if (tt->writes.iostate == IOSTATE_INITIAL)
    {
      BOOL status;
      int err;
	  return mywrite_tun(tt,BPTR (buf),BLEN(buf));
      ...
      if (status) /* operation completed immediately? */
	{
	  ...
	  dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Write immediate return [%d,%d]",
	       BLEN (&tt->writes.buf),
	       (int) tt->writes.size);	       
	}
      else
	  ...
	  if (err == ERROR_IO_PENDING) /* operation queued? */
	    {
	      ...
	      dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Write queued [%d]",
		   BLEN (&tt->writes.buf));
	    }
	  else /* error occurred */
	    {
	      ...
	      tt->writes.iostate = IOSTATE_IMMEDIATE_RETURN;
	      tt->writes.status = err;
	      dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Write error [%d] : %s",
		   BLEN (&tt->writes.buf),
	      ...
	      struct buffer *buf)
{
  ...
  switch (io->iostate)
      ...
      if (status)
	{
	  ...
	  dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Completion success [%d]", ret);
	}
      else
	{
	  ...
	  if (GetLastError() != ERROR_IO_INCOMPLETE)
	    {
		 ...
		 then DON'T execute this code */
	      io->iostate = IOSTATE_INITIAL;
	      ASSERT (ResetEvent (io->overlapped.hEvent));
	      msg (D_WIN32_IO | M_ERRNO, "MYWIN32 I/O: TAP Completion error");
	    }
	}
      break;
      ...
      if (io->status)
	{
	  ...
	  ret = -1;
	  msg (D_WIN32_IO | M_ERRNO, "MYWIN32 I/O: TAP Completion non-queued error");
	}
      else
	{
	  ...
	  if (buf)
	    *buf = io->buf;
	  ret = io->size;
	  dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Completion non-queued success [%d]", ret);
	}
      break;
    ...
    case IOSTATE_INITIAL: /* were we called without proper queueing? */
      ...
      ret = -1;
      dmsg (D_WIN32_IO, "MYWIN32 I/O: TAP Completion BAD STATE");
      break;
...
verify_255_255_255_252 (in_addr_t local, in_addr_t remote)
{
  ...
  if ((local & (~mask)) != (remote & (~mask)))
    {
      err = "must exist within the same 255.255.255.252 subnet.  This is a limitation of --dev tun when used with the TAP-MYWIN32 driver";
      goto error;
    }
  if ((local & mask) == 0
      || (local & mask) == 3
      ...
      || (remote & mask) == 3)
    {
      err = "cannot use the first or last address within a given 255.255.255.252 subnet.  This is a limitation of --dev tun when used with the TAP-MYWIN32 driver";
      goto error;
    }
...
show_tap_win32_adapters (int msglev, int warnlev)
{
  ...
  msg (msglev, "Available TAP-MYWIN32 adapters [name, GUID]:");
...
delete_temp_addresses (DWORD index)
{
  ...
  if (a)
      ...
      while (ip)
	{
	  ...
	  if ((status = DeleteIPAddressMy ((ULONG) context)) == NO_ERROR)
	    {
...
get_adapter_index_method_1 (const char *guid)
{
  ...
  wchar_t wbuf[256];
return 0;
  sprintf (wbuf, SIZE (wbuf), L"\\DEVICE\\TCPIP_%S", guid);
  wbuf [SIZE(wbuf) - 1] = 0;
...
open_tun (const char *dev, const char *dev_type, const char *dev_node, bool ipv6, struct tuntap *tt)
{
 ...
 char actual_buffer[256]="tun0";
	...
	if (!device_guid)
	  msg (M_FATAL, "TAP-Win32 adapter '%s' not found", dev_node);
        ...
        openvpn_snprintf (device_path, sizeof(device_path), "%s%s%s",
		          ...
		          TAPSUFFIX);
#else 
		  sprintf(device_path,"%s\\dev\\tun0","c:");
	#endif
        tt->hand = CreateFileA (
			device_path,
  ...
  msg (M_INFO, "TAP-MYWIN32 device [%s] opened: %s", tt->actual_name, device_path);
...
//#ifndef SIMULATE_DHCP_FAILED /* this code is disabled to simulate bad DHCP negotiation */
#ifdef SIMULATE_DHCP_FAILED /* this code is disabled to simulate bad DHCP negotiation */
  ...
  /* possibly use IP Helper API to set IP address on adapter */
  {
	...
	if ((status = AddIPAddressMy (htonl(tt->local),
...
close_tun (struct tuntap *tt)
{
  ...
  if (tt)
	...
	{
	  DWORD status;
	  if ((status = DeleteIPAddressMy (tt->ipapi_context)) != NO_ERROR)
	    {
	      msg (M_WARN, "Warning: DeleteIPAddressMy[%u] failed on TAP-Win32 adapter, status=%u : %s",
...
int openvpn_status (struct tuntap* tt,int status)
{
	//printf("\n openvpn_status");
	if(tt->statusCallP)
	{
...
int parseEtherNetPacket(struct tuntap* tt, uint8_t *buf, int len)
{
	...
	{	
		...
		if(ih->protocol == OPENVPN_IPPROTO_UDP)
		{
			...
			srcport = opP->source;
			dstport = opP->dest;//ntohs(opP->dest);
			//actuallen = (int)(len- ( sizeof(struct openvpn_iphdr)+sizeof(struct openvpn_udphdr)));//ntohs(opP->len);
			//	printf("\npacket ipsrc= %s ,ipdest = %s %d %d %d %d,passlen=%d  ,orglen=%d\n\n\n",ip,inet_ntoa(y),ntohs(opP->source),ntohs(opP->dest),
			//	ntohs(opP->check),ntohs(opP->len),actuallen,len);
			//dataP = buf + sizeof(struct openvpn_iphdr)+sizeof(struct openvpn_udphdr);
