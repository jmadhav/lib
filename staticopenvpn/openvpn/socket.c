...
#ifndef MYWIN32 /* using SO_REUSEADDR on Windows will cause bind to succeed on port conflicts! */
...
socket_recv_queue (struct link_socket *sock, int maxsize)
{
  if (sock->reads.iostate == IOSTATE_INITIAL)
      ...
      if (!status) /* operation completed immediately? */
	{
	  ...
	  dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Receive immediate return [%d,%d]",
	       (int) wsabuf[0].len,
	       (int) sock->reads.size);	       
	}
      else
	  ...
	  if (status == WSA_IO_PENDING) /* operation queued? */
	    {
	      ...
	      dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Receive queued [%d]",
		   (int) wsabuf[0].len);
	    }
	  else /* error occurred */
	    {
	      ...
	      sock->reads.iostate = IOSTATE_IMMEDIATE_RETURN;
	      sock->reads.status = status;
	      dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Receive error [%d]: %s",
		   (int) wsabuf[0].len,
...
socket_send_queue (struct link_socket *sock, struct buffer *buf, const struct link_socket_actual *to)
{
  if (sock->writes.iostate == IOSTATE_INITIAL)
      ...
      if (!status) /* operation completed immediately? */
	{
	  ...
	  dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Send immediate return [%d,%d]",
	       (int) wsabuf[0].len,
	  ...
	  if (status == WSA_IO_PENDING) /* operation queued? */
	    {
	      ...
	      dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Send queued [%d]",
		   (int) wsabuf[0].len);
	    }
	  else /* error occurred */
	    {
	      ...
	      dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Send error [%d]: %s",
		   (int) wsabuf[0].len,
		   strerror_win32 (status, &gc));
		 ...
		 struct link_socket_actual *from)
{
  ...
  switch (io->iostate)
      ...
      if (status)
	{
	  ...
	  dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Completion success [%d]", ret);
	}
      else
	{
	  ...
	  if (WSAGetLastError() != WSA_IO_INCOMPLETE)
	    {
	      ...
	      io->iostate = IOSTATE_INITIAL;
	      ASSERT (ResetEvent (io->overlapped.hEvent));
	      msg (D_WIN32_IO | M_ERRNO_SOCK, "MYWIN32 I/O: Socket Completion error");
	    }
	}
      break;
      ...
      if (io->status)
	{
	  ...
	  ret = -1;
	  msg (D_WIN32_IO | M_ERRNO_SOCK, "MYWIN32 I/O: Socket Completion non-queued error");
	}
      else
	{
	  ...
	  if (buf)
	    *buf = io->buf;
	  ret = io->size;
	  dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Completion non-queued success [%d]", ret);
	}
      break;
    ...
    case IOSTATE_INITIAL: /* were we called without proper queueing? */
      ...
      ret = -1;
      dmsg (D_WIN32_IO, "MYWIN32 I/O: Socket Completion BAD STATE");
      break;
