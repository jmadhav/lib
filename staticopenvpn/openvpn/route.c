...
add_route (struct route *r, const struct tuntap *tt, unsigned int flags, const struct env_set *es)
{
  ...
  argv_msg (D_ROUTE, &argv);
	status = 0;//openvpn_execve_check (&argv, es, 0, "ERROR: Linux route add command failed");

#elif defined (MYWIN32)
  ...
  if ((flags & ROUTE_METHOD_MASK) == ROUTE_METHOD_IPAPI)
  ...
  else if ((flags & ROUTE_METHOD_MASK) == ROUTE_METHOD_EXE)
    {
      netcmd_semaphore_lock ();
		status =0;// openvpn_execve_check (&argv, es, 0, "ERROR: Windows route add command failed");
      ...
      if (!status)
	{
	  ...
	  netcmd_semaphore_lock ();
		status =0;// openvpn_execve_check (&argv, es, 0, "ERROR: Windows route add command failed [adaptive]");
	  netcmd_semaphore_release ();
	}
    }
  else
...
add_route_ipapi (const struct route *r, const struct tuntap *tt)
{
  ...
  if (if_index != ~0)
	...
	msg (M_WARN, "Warning: address %s is not a network address in relation to netmask %s",
      ...
      status = NO_ERROR;//CreateIpForwardEntry (&fr);
  ...
  if (if_index != ~0)
    {
      ...
      status = NO_ERROR;//DeleteIpForwardEntry (&fr);
