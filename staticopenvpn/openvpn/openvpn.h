...
//typedef unsigned int (*readDataCallback)(char*host,int port ,char *data,int len);
...
# define CM_CHILD_TCP      4 /* child context of a CM_TOP or CM_THREAD */
  ...
  struct context_2 c2;
	readwriteDataCallback readCallbackP;		
	readwriteDataCallback writeCallbackP;	
	void *uData;
...
int initAndStartOpenVpn (OpenvpnInterfaceType *openVpntP,int argc, char *argv[]);
int dataSent(char *server,int port,char *data,int len);
unsigned int readwriteDataCallbackL (char*host,int *portP ,unsigned char *data,int *lenP);
int testVPN();
