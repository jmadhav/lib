...
initialization_sequence_completed (struct context *c, const unsigned int flags)
{
  ...
  {	  
    msg (M_INFO, "%s", message);
	  ...
	  if(c->c1.tuntap)
	{
		openvpn_status(c->c1.tuntap,status);
  ...
  if (!c->c1.tuntap)
    {
      ...
      do_init_tun (c);
		c->c1.tuntap->readCallbackP = c->readCallbackP;
		c->c1.tuntap->writeCallbackP = c->writeCallbackP;
		c->c1.tuntap->writeCallbackP = c->writeCallbackP;
		c->c1.tuntap->statusCallP = c->statusCallP;
		c->c1.tuntap->uData = c->uData;
